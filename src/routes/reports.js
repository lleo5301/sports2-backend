 ...
 */
router.post('/', _validateReportCreate, _handleValidationErrors, checkPermission('reports_create'), async (req, res) => {
  try {
 ...
 */
router.put('/byId/:id', _validateReportUpdate, _handleValidationErrors, checkPermission('reports_edit'), async (req, res) => {
  try {
    // Database: Find report with team scoping for security
    const report = await Report.findOne({
      where: {
...
router.post('/scouting', async (req, res) => {
  try {
    ...
    const {
      player_id: _pid, prospect_id: _prid, // already handled above
...
router.put('/scouting/:id', async (req, res) => {
  try {
    ...
    for (const field of allowedFields) {
      if (field in convertedBody) {
        updateData[field] = convertedBody[field];
      }
    }

    // Database: Apply partial update with whitelisted fields
